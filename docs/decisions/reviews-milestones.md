...
**Branch:** `usman/reviews-test` (M1 UI + M2 data + M3 forms)
...
| Token-driven URL state | Done (M3) | App Proxy + `reviews_link_state` cookie |
...
**TEMP-DEBUG:** removed at M3 (`review-product-icon.liquid`).
...
**M3:** App Proxy sets `reviews_link_state` cookie → theme JS renders expired or used copy. Theme editor **Preview state** for design-mode QA only.
...
**Scope:** App Proxy submit pipeline + §5.1–§5.5 forms + §5.5b cookie routing.  
**Architecture doc:** [`reviews-milestone-3.md`](./reviews-milestone-3.md)  
**App repo:** `myduggie-app` (separate from theme)

### M3 deliverables checklist

| Deliverable | Status | Notes |
|-------------|--------|-------|
| App Proxy HMAC on every request | Done (app) | `myduggie-app` |
| Signed token 60-day TTL + single-use | Done (app) | `REVIEWS_TOKEN_SECRET` env — **not** shop metafield |
| `/enter` → session cookie → form redirect | Done (app) | Never link directly to `/pages/review-*?token=` |
| `/context` order prefill | Done (app + theme JS) | owns, reviewer_name |
| `/submit` → Review metaobject `approved` | Done (app) | owns from session; `is_sample` from customer metafield |
| §5.1–§5.4 form pages | Done (theme) | Locked copy verbatim |
| §5.5 thank-you | Done (theme) | |
| §5.5b cookie-driven inactive | Done (theme) | `reviews_link_state` cookie + JS |
| `/generate-token` gate tool | Done (app) | Klaviyo URL contract in milestone-3 doc |
| Remove shop JSON sample data path | Done (M2) | Metaobjects only |
| Remove TEMP-DEBUG on Owns icons | Done (M3) | |

### Admin pages (create in Shopify Admin)
...
Klaviyo entry URL: `https://{shop}/apps/reviews/enter?order=…&token=…&type=buyer|recipient|gifter|accessory`
...
| Item | Notes |
|------|--------|
| Product rollup webhook (`review_count` / `average_rating`) | + theme read on PDP |
| §5.7 / §5.8 recipient & gifter cards | |
| §5.10–5.12 PDP reviews surface | Product-scoped queries |
| Klaviyo production flows | Parameterized slots; Brian copy |
| Gifter `gave` from Gift Flow Closing-Loop | Stub empty at M3 gate |
| Photo lightbox | |
| Returns badge live fulfillment lookup | Replace TEMP-DEV `answers.order_returned` |
...
snippets/
  ...
  review-form-stars-input.liquid
  review-form-photo-zone.liquid
  aggregate-rating-block.liquid
...
**Submission app (separate repo):** `myduggie-app` — App Proxy, token signing, Admin API writes. See [`reviews-milestone-3.md`](./reviews-milestone-3.md).
